...
- **Publishable change → changeset.** A PR that touches a publishable package (`packages/*` except `typescript-config`) needs a `.changeset/*.md`: run `pnpm changeset`, pick the bump, write the one-line consumer note. CI's `changeset status` fails the PR without one. `apps/www` and config are in the changeset `ignore` list, so app- and doc-only work needs nothing. A deliberate no-release change — internal refactor, test-only, or an in-package `docs/`/ADR edit — is acknowledged with `pnpm changeset --empty`. Per-package `CHANGELOG.md` is generated from these at release; never hand-edit it.
