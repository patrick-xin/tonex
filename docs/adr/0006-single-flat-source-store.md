...
**Decision:** Source state is **one zustand store** with a **flat top-level shape**. Categorisation lives in field name prefixes (`md3TokenOverrides`, `shadcnRoleBindings`, `surfaceAlgo`, `surfaceTintLevel`, `cmfSecondSourceHex`), never in nested struct slots. Mode-keyed records (`{ light, dark }`) are values, not slices. Actions are bundled under a single `actions` key with stable identity.
...
- `selectPortable(state)` strips `_hydrated` and `actions` to produce the persisted shape (`PortableTheme`). The flat layout makes that strip mechanical — no traversal, no slot-map.
