...
- A citation check enforces Decision 8 — every `ADR-N c.M` / `ADR-N amendment <date>` cited in code or docs must resolve to a live anchor. This is the harness that makes aggressive doc refactors safe, and it is the one part of this ADR that is code; it is filed as a follow-on (this is a docs-only session).
