...
How to **gate** the palette: what `check` blocks vs. warns, how to read its exit codes, and how to nudge a single token. Companion to [SKILL.md](../SKILL.md), this is the final gate before delivery, not an afterthought. The authoritative thresholds are always `describe`'s `contrast` / `exitCodes` fields; this file is the interpretation they can't carry.

## Contrast policy (what `check` blocks vs. warns)

This table and the thresholds below mirror `describe`'s `contrast` field (authoritative for the numbers). What's here is the *interpretation* the field can't carry.
...
- The theme-free forms (`<fg> <bg>` and `--pairs` without `--seed`) accept each color as a **6-digit hex or canonical `oklch(L C H)`**. An out-of-gamut oklch is gamut-mapped to sRGB before scoring, and the verdict echoes that projected hex (the color actually scored).
...
- The remedy ladder for a text failure: `--find-contrast` to get the minimum `--contrast` → re-generate → re-check. If AAA is `UNREACHABLE`, report that and ask whether AA is acceptable; do not silently downgrade.

## Exit-code playbook

The codes mirror `describe`'s `exitCodes` (authoritative). The *response* to each is the judgment below.
...
```
adjust --seed '#3b82f6' --shifts '[{"mode":"dark","token":"primary","dTone":5}]'
# → dark  primary  #6e9fff → #86adff   req t+5 c+0   got t+4.99 c-6.78
```
It shifts named tokens by a relative ±HCT delta and prints before/after plus the **gamut-clamped achieved** delta (so a clamp shows up in the numbers). It **never gates contrast**, and it **doesn't persist** — so the whole-theme `check --seed` gate re-derives clean from the seed and won't see your shift.

Treat `adjust` as exploratory by default. If you deliberately deliver adjusted values, the artifact needs both commands: the base `generate` recipe plus the exact `adjust` command or an adjacent note listing the adjusted token/value overrides. Verify each adjusted pairing *ad-hoc*: feed the printed after-value and the color it pairs against into `check <fg> <bg>` (e.g. `check '#86adff' '<paired-bg>'`). The token names are the bare md roles `--to colors` prints; see [token naming](integrations/CUSTOM-TARGET.md#token-naming-across-surfaces).
...
- **Don't** treat the literal pin as a coordinated part of the system. It sits **outside** tonex's contrast guarantee — only the fill↔foreground pair this command returns is gated. Verify any *other* pairing that literal color participates in with a real pair, for example `check '#ffffff' '#6750a4'`.
