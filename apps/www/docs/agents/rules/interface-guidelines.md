...
- **`motion/react`: import the lazy `m`, never `motion`.** `LazyMotion features={domAnimation}` is mounted once at the root (`app/layout.tsx`), so `m` animates everywhere — never re-wrap a subtree in its own `LazyMotion`, and don't import `motion` (it pulls the full bundle the root deliberately defers).
- **Reduced motion is global — don't re-derive it for transform/opacity animations.** The root's `MotionConfig reducedMotion="user"` already makes every `m` component honor `prefers-reduced-motion`: transform/layout animations snap to their target, opacity still fades. An `m` element with a translate/scale/fade entrance needs **no** `useReducedMotion()` gate. Reach for the hook only to gate motion the config can't see — a shader/Lottie `speed`, a JS-driven loop, or an infinite ambient animation (see `shader-hero`, `site-footer`). Gating a plain transform on it is the duplication to drop.
