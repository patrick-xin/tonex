...
Surface strength is **per mode** — light and dark each carry their own level — while the algorithm and the chosen palette are shared. See [Light & dark](/docs/web-app/light-dark). In the web app these live in Settings; from the CLI they're `--tint` / `--tint-palette` and `--desaturate` (mutually exclusive).
