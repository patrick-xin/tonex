...
`--to yaml` emits a [DESIGN.md](https://github.com/google-labs-code/design.md) `colors:` block. It is the portable semantic contract for tools that can consume DESIGN.md. Tonex exports `shadcn` and `json` natively; for other tools, the roles (`primary`, `on-surface`, and so on) mean the same thing wherever they land. See [How Tonex thinks](/docs/how-tonex-thinks#the-semantic-source-is-the-contract) and the [skill](/docs/skills) that applies a consistent mapping.
