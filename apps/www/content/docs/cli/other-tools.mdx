...
Do not invent colors during the mapping step. If a mapped tool needs a different hierarchy, remap Tonex roles first; if a checked pair fails, use a Tonex remedy such as `--contrast` or choose a different pairing. See [How Tonex thinks](/docs/key-concepts/how-tonex-thinks#the-semantic-source-is-the-contract).
