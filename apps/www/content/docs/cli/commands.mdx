...
| Flag | Default | Details |
| --- | --- | --- |
| `--variant` | `cmf` | `cmf`, `tonalSpot`, `neutral`, `fidelity`, `content`, `vibrant`, `expressive`, `rainbow`, `fruitSalad`, or `monochrome`. See [Variants](/docs/key-concepts/variants). |
...
| Flag | Default | Details |
| --- | --- | --- |
| `--to` | `shadcn` | `colors`, `shadcn`, `yaml`, or `json`. See [Targets & formats](/docs/cli/targets-formats); for non-native tools, see [Mapping to Other Tools](/docs/cli/other-tools). |
...
| `--tint-palette` | `zinc` | `slate`, `gray`, `zinc`, `neutral`, `stone`, `taupe`, `mauve`, `mist`, or `olive`. Used when `--tint` is set. |
