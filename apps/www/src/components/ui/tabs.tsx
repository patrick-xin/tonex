...
function Tabs({ className, orientation = 'horizontal', ...props }: BaseTabs.Root.Props) {
  return (
    <BaseTabs.Root
      className={cn('flex gap-2 flex-col data-[orientation=vertical]:flex-row', className)}
      data-slot="tabs"
  ...
  return (
    <BaseTabs.Tab
      className={cn(
        ...
        'text-sm font-medium text-on-surface data-active:text-on-primary-container',
...
function TabsListContent({
  ...
  indicatorClassName,
  ...props
}: BaseTabs.List.Props & { indicatorClassName?: string }) {
  return (
    <BaseTabs.List
      className={cn(
        'relative z-0 inline-flex items-center rounded-md gap-1 px-1.5 py-1 bg-surface-container text-on-surface-variant min-h-9',
    ...
    >
      ...
      <TabsIndicator
        className={cn(
          'rounded-md bg-primary-container',
