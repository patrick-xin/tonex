...
import type { ToastManagerUpdateOptions, TooltipPositionerProps } from '@base-ui/react'
import {
  Toast as BaseToast,
...
const toast = {
  ...
  error: (options?: ToastOptions) => stackedManager.add({ ...getOptions(options), type: 'error' }),
  info: (options?: ToastOptions) => stackedManager.add({ ...getOptions(options), type: 'info' }),
  promise: <Value,>(
    ...
    return stackedManager.promise(promise, {
      ...
      success: (val) => {
        const resolved =
          typeof options.success === 'function' ? options.success(val) : options.success
        return normalize(resolved)
      },
    })
  },
...
const ToastTitle = ({ children, ...props }: BaseToast.Title.Props) => {
  return (
    <BaseToast.Title className="text-sm font-semibold" data-slot="toast-title" {...props}>
      {children}
    </BaseToast.Title>
  )
}

const ToastDescription = ({ children, ...props }: BaseToast.Description.Props) => {
  return (
    <BaseToast.Description className="text-sm" data-slot="toast-description" {...props}>
      {children}
    ...
    <BaseToast.Portal>
      <ToastViewport className="toast-viewport" data-position={position}>
        {toasts.map((toast) => {
          ...
          return (
            <BaseToast.Root
              className={cn('toast-root', !isCustomContent && toastVariants({ type: toastType }))}
                ...
                {isCustomContent ? (
                  ...
                  <div className="flex items-start gap-3 p-4">
                    ...
                    <div className="flex flex-col flex-wrap flex-1 gap-2">
                      ...
                      {toast.actionProps && (
                        <ToastAction className={cn(buttonStyles({ size: 'sm' }), 'w-fit')}>
                      ...
                      {toast.data?.closable && (
                        <ToastClose
                          className={cn(
                            ...
                            toastType === 'warning' && 'hover:bg-yellow-500/20!',
...
const AnchoredToast = () => {
  ...
  return (
    <BaseToast.Portal>
      <ToastViewport className="fixed inset-0 pointer-events-none z-100">
        {toasts.map((toast) => {
          const isCustomContent = toast.data?.customContent
          return (
...
export { ToastAction, ToastArrow, ToastClose, ToastDescription, Toaster, ToastTitle, toast }
