...
import { Legend as RechartsLegend, Tooltip as RechartsTooltip, ResponsiveContainer } from 'recharts'
    ...
    <ChartContext.Provider value={{ config }}>
      <div
        className={cn(
          ...
          "[&_.recharts-cartesian-axis-tick_text]:fill-on-surface-variant [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-outline/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-outline [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-outline [&_.recharts-radial-bar-background-sector]:fill-surface-container-high [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-surface-container-high [&_.recharts-reference-line_[stroke='#ccc']]:stroke-outline [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-text]:fill-on-surface-variant/90",
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  return (
    <style
      // biome-ignore lint/security/noDangerouslySetInnerHtml: shadcn chart pattern — content is internal config (theme prefixes + token names), not user input
  ...
  return (
    <div
      className={cn(
        ...
        'grid min-w-16 items-start gap-1.5 rounded-sm px-2.5 py-1.5 shadow-xl',
        className,
      )}
      ...
      <div className="grid gap-1.5">
        {payloadItems.map((item, index) => {
          ...
          return (
            <div
              className={cn(
                '[&>svg]:text-on-surface-variant flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5',
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            className={cn(
              '[&>svg]:text-on-surface-variant flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3',
            )}
            key={item.value}
          >
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  if (key in payloadItem && typeof payloadItem[key as keyof ChartItem] === 'string') {
