...
function SheetTrigger<Payload>({ className, ...props }: BaseSheet.Trigger.Props<Payload>) {
  return <BaseSheet.Trigger className={className} data-slot="sheet-trigger" {...props} />
}

function SheetPortal({ className, ...props }: BaseSheet.Portal.Props) {
  return <BaseSheet.Portal className={className} data-slot="sheet-portal" {...props} />
}
...
function SheetBackdrop({ className, ...props }: BaseSheet.Backdrop.Props) {
  return (
    <BaseSheet.Backdrop
      className={cn(
        'fixed inset-0 min-h-dvh bg-scrim/20 backdrop-blur-xs animate-fade',
...
function SheetPopup({ className, children, ...props }: BaseSheet.Popup.Props) {
  return (
    <BaseSheet.Popup className={cn('surface-dialog', className)} data-slot="sheet-popup" {...props}>
      {children}
    </BaseSheet.Popup>
  )
}
...
function SheetDescription({ className, ...props }: BaseSheet.Description.Props) {
  return (
    <BaseSheet.Description
      className={cn('text-on-surface text-sm', className)}
...
function SheetClose({ className, ...props }: BaseSheet.Close.Props) {
  return <BaseSheet.Close className={className} data-slot="sheet-close" {...props} />
}

const popupVariants = cva(
  'relative flex flex-col gap-4 p-4 sm:p-6 surface-dialog max-h-full w-full overflow-hidden',
  {
    defaultVariants: {
      side: 'right',
    },
...
interface SheetContentProps extends BaseSheet.Popup.Props, VariantProps<typeof popupVariants> {
  ...
  return (
    <SheetPortal>
      ...
      <SheetViewport inset={inset} side={side}>
        <BaseSheet.Popup
          className={cn(popupVariants({ side }), inset && 'rounded-md overlay-outline', className)}
