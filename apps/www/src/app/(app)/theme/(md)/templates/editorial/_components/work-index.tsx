...
// A detached handle bridges the row triggers to one shared sheet: every row is a
// real SheetTrigger carrying its project as `payload`, and the sheet's
// render-function child reads that payload. Base UI keeps the active payload
// mounted through the close animation, so the panel slides out *with* its content
// instead of blinking empty (the prior onClick-driven `selectedProject` state
// cleared on close-start, which is what made the sheet jitter).
        ...
        <div className="flex flex-wrap gap-1.5 bg-surface-container p-1 rounded-full border border-outline-variant">
          {(['all', 'branding', 'editorial', 'digital', 'type'] as const).map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-mono tracking-wider transition-colors cursor-pointer uppercase ${
                activeCategory === cat
          ...
          {filteredProjects.map((proj) => (
            ...
            >
              <SheetTrigger
                ...
                payload={proj}
                render={<div />}
                className="grid grid-cols-12 items-center w-full py-5 border-b border-outline-variant hover:bg-surface-container transition-colors cursor-pointer px-3 group text-left"
              >
