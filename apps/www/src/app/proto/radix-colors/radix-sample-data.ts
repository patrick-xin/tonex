// VENDORED from @radix-ui/colors@3.0.0 (solid light + dark, 12 steps each).
// Generated, not hand-typed — accurate. Throwaway prototype data for the picker
// pattern lab; delete the proto/radix-colors/ folder to remove. A real build
// would consume this through @tonex/core/data (the ColorSystem slot), not here.
import { MODES, type Mode } from '@tonex/core'

export type RadixBand = 'background' | 'component' | 'border' | 'solid' | 'text'

export type StepRole = { step: number; band: RadixBand; label: string }

// why: each of the 12 steps has a JOB — Tailwind's 50→950 carries none of this.
// The picker exists to make this legible, so the metadata is first-class data.
export const STEP_ROLES: StepRole[] = [
  { step: 1, band: 'background', label: 'App background' },
  { step: 2, band: 'background', label: 'Subtle background' },
  { step: 3, band: 'component', label: 'UI element background' },
  { step: 4, band: 'component', label: 'Hovered UI element' },
  { step: 5, band: 'component', label: 'Active / selected element' },
  { step: 6, band: 'border', label: 'Subtle border / separator' },
  { step: 7, band: 'border', label: 'UI element border / focus ring' },
  { step: 8, band: 'border', label: 'Hovered UI element border' },
  { step: 9, band: 'solid', label: 'Solid background' },
  { step: 10, band: 'solid', label: 'Hovered solid background' },
  { step: 11, band: 'text', label: 'Low-contrast text' },
  { step: 12, band: 'text', label: 'High-contrast text' },
]

export type BandDef = { key: RadixBand; name: string; steps: number[]; blurb: string }

export const BANDS: BandDef[] = [
  { key: 'background', name: 'Backgrounds', steps: [1, 2], blurb: 'Page & subtle fills' },
  { key: 'component', name: 'Component', steps: [3, 4, 5], blurb: 'Element bg / hover / active' },
  { key: 'border', name: 'Borders', steps: [6, 7, 8], blurb: 'Separators, borders, focus' },
  { key: 'solid', name: 'Solid', steps: [9, 10], blurb: 'The brand color + its hover' },
  { key: 'text', name: 'Text', steps: [11, 12], blurb: 'Low- & high-contrast text' },
]

// why: step 9 is the identity step — most-saturated, the "this is the color"
// swatch (Radix's analog to Tailwind 500). Pickers default the eye here.
export const IDENTITY_STEP = 9

export function roleFor(step: number): StepRole {
  return STEP_ROLES[step - 1]
}

export function bandFor(step: number): BandDef {
  const role = roleFor(step)
  return BANDS.find((b) => b.key === role.band) as BandDef
}

export type GroupKey =
  | 'reds-pinks'
  | 'purples'
  | 'blues'
  | 'greens'
  | 'yellows'
  | 'metals'
  | 'grays'

export type GroupDef = { key: GroupKey; label: string; scales: string[] }

export const GROUPS: GroupDef[] = [
  {
    key: 'reds-pinks',
    label: 'Reds & pinks',
    scales: ['tomato', 'red', 'ruby', 'crimson', 'pink', 'plum'],
  },
  { key: 'purples', label: 'Purples', scales: ['purple', 'violet', 'iris', 'indigo'] },
  { key: 'blues', label: 'Blues', scales: ['blue', 'cyan', 'sky'] },
  { key: 'greens', label: 'Greens', scales: ['teal', 'jade', 'green', 'grass', 'mint', 'lime'] },
  { key: 'yellows', label: 'Yellows & oranges', scales: ['yellow', 'amber', 'orange'] },
  { key: 'metals', label: 'Browns & metals', scales: ['brown', 'bronze', 'gold'] },
  { key: 'grays', label: 'Grays', scales: ['gray', 'mauve', 'slate', 'sage', 'olive', 'sand'] },
]

export type RadixScale = {
  name: string
  kind: 'accent' | 'gray'
  group: GroupKey
  // index 0 = step 1 … index 11 = step 12, per mode
  ramp: Record<Mode, string[]>
}

export function stepHex(scale: RadixScale, mode: Mode, step: number): string {
  return scale.ramp[mode][step - 1]
}

export const SCALES: RadixScale[] = [
  {
    name: 'tomato',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fffcfc',
        '#fff8f7',
        '#feebe7',
        '#ffdcd3',
        '#ffcdc2',
        '#fdbdaf',
        '#f5a898',
        '#ec8e7b',
        '#e54d2e',
        '#dd4425',
        '#d13415',
        '#5c271f',
      ],
      dark: [
        '#181111',
        '#1f1513',
        '#391714',
        '#4e1511',
        '#5e1c16',
        '#6e2920',
        '#853a2d',
        '#ac4d39',
        '#e54d2e',
        '#ec6142',
        '#ff977d',
        '#fbd3cb',
      ],
    },
  },
  {
    name: 'red',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fffcfc',
        '#fff7f7',
        '#feebec',
        '#ffdbdc',
        '#ffcdce',
        '#fdbdbe',
        '#f4a9aa',
        '#eb8e90',
        '#e5484d',
        '#dc3e42',
        '#ce2c31',
        '#641723',
      ],
      dark: [
        '#191111',
        '#201314',
        '#3b1219',
        '#500f1c',
        '#611623',
        '#72232d',
        '#8c333a',
        '#b54548',
        '#e5484d',
        '#ec5d5e',
        '#ff9592',
        '#ffd1d9',
      ],
    },
  },
  {
    name: 'ruby',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fffcfd',
        '#fff7f8',
        '#feeaed',
        '#ffdce1',
        '#ffced6',
        '#f8bfc8',
        '#efacb8',
        '#e592a3',
        '#e54666',
        '#dc3b5d',
        '#ca244d',
        '#64172b',
      ],
      dark: [
        '#191113',
        '#1e1517',
        '#3a141e',
        '#4e1325',
        '#5e1a2e',
        '#6f2539',
        '#883447',
        '#b3445a',
        '#e54666',
        '#ec5a72',
        '#ff949d',
        '#fed2e1',
      ],
    },
  },
  {
    name: 'crimson',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fffcfd',
        '#fef7f9',
        '#ffe9f0',
        '#fedce7',
        '#facedd',
        '#f3bed1',
        '#eaacc3',
        '#e093b2',
        '#e93d82',
        '#df3478',
        '#cb1d63',
        '#621639',
      ],
      dark: [
        '#191114',
        '#201318',
        '#381525',
        '#4d122f',
        '#5c1839',
        '#6d2545',
        '#873356',
        '#b0436e',
        '#e93d82',
        '#ee518a',
        '#ff92ad',
        '#fdd3e8',
      ],
    },
  },
  {
    name: 'pink',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fffcfe',
        '#fef7fb',
        '#fee9f5',
        '#fbdcef',
        '#f6cee7',
        '#efbfdd',
        '#e7acd0',
        '#dd93c2',
        '#d6409f',
        '#cf3897',
        '#c2298a',
        '#651249',
      ],
      dark: [
        '#191117',
        '#21121d',
        '#37172f',
        '#4b143d',
        '#591c47',
        '#692955',
        '#833869',
        '#a84885',
        '#d6409f',
        '#de51a8',
        '#ff8dcc',
        '#fdd1ea',
      ],
    },
  },
  {
    name: 'plum',
    kind: 'accent',
    group: 'reds-pinks',
    ramp: {
      light: [
        '#fefcff',
        '#fdf7fd',
        '#fbebfb',
        '#f7def8',
        '#f2d1f3',
        '#e9c2ec',
        '#deade3',
        '#cf91d8',
        '#ab4aba',
        '#a144af',
        '#953ea3',
        '#53195d',
      ],
      dark: [
        '#181118',
        '#201320',
        '#351a35',
        '#451d47',
        '#512454',
        '#5e3061',
        '#734079',
        '#92549c',
        '#ab4aba',
        '#b658c4',
        '#e796f3',
        '#f4d4f4',
      ],
    },
  },
  {
    name: 'purple',
    kind: 'accent',
    group: 'purples',
    ramp: {
      light: [
        '#fefcfe',
        '#fbf7fe',
        '#f7edfe',
        '#f2e2fc',
        '#ead5f9',
        '#e0c4f4',
        '#d1afec',
        '#be93e4',
        '#8e4ec6',
        '#8347b9',
        '#8145b5',
        '#402060',
      ],
      dark: [
        '#18111b',
        '#1e1523',
        '#301c3b',
        '#3d224e',
        '#48295c',
        '#54346b',
        '#664282',
        '#8457aa',
        '#8e4ec6',
        '#9a5cd0',
        '#d19dff',
        '#ecd9fa',
      ],
    },
  },
  {
    name: 'violet',
    kind: 'accent',
    group: 'purples',
    ramp: {
      light: [
        '#fdfcfe',
        '#faf8ff',
        '#f4f0fe',
        '#ebe4ff',
        '#e1d9ff',
        '#d4cafe',
        '#c2b5f5',
        '#aa99ec',
        '#6e56cf',
        '#654dc4',
        '#6550b9',
        '#2f265f',
      ],
      dark: [
        '#14121f',
        '#1b1525',
        '#291f43',
        '#33255b',
        '#3c2e69',
        '#473876',
        '#56468b',
        '#6958ad',
        '#6e56cf',
        '#7d66d9',
        '#baa7ff',
        '#e2ddfe',
      ],
    },
  },
  {
    name: 'iris',
    kind: 'accent',
    group: 'purples',
    ramp: {
      light: [
        '#fdfdff',
        '#f8f8ff',
        '#f0f1fe',
        '#e6e7ff',
        '#dadcff',
        '#cbcdff',
        '#b8baf8',
        '#9b9ef0',
        '#5b5bd6',
        '#5151cd',
        '#5753c6',
        '#272962',
      ],
      dark: [
        '#13131e',
        '#171625',
        '#202248',
        '#262a65',
        '#303374',
        '#3d3e82',
        '#4a4a95',
        '#5958b1',
        '#5b5bd6',
        '#6e6ade',
        '#b1a9ff',
        '#e0dffe',
      ],
    },
  },
  {
    name: 'indigo',
    kind: 'accent',
    group: 'purples',
    ramp: {
      light: [
        '#fdfdfe',
        '#f7f9ff',
        '#edf2fe',
        '#e1e9ff',
        '#d2deff',
        '#c1d0ff',
        '#abbdf9',
        '#8da4ef',
        '#3e63dd',
        '#3358d4',
        '#3a5bc7',
        '#1f2d5c',
      ],
      dark: [
        '#11131f',
        '#141726',
        '#182449',
        '#1d2e62',
        '#253974',
        '#304384',
        '#3a4f97',
        '#435db1',
        '#3e63dd',
        '#5472e4',
        '#9eb1ff',
        '#d6e1ff',
      ],
    },
  },
  {
    name: 'blue',
    kind: 'accent',
    group: 'blues',
    ramp: {
      light: [
        '#fbfdff',
        '#f4faff',
        '#e6f4fe',
        '#d5efff',
        '#c2e5ff',
        '#acd8fc',
        '#8ec8f6',
        '#5eb1ef',
        '#0090ff',
        '#0588f0',
        '#0d74ce',
        '#113264',
      ],
      dark: [
        '#0d1520',
        '#111927',
        '#0d2847',
        '#003362',
        '#004074',
        '#104d87',
        '#205d9e',
        '#2870bd',
        '#0090ff',
        '#3b9eff',
        '#70b8ff',
        '#c2e6ff',
      ],
    },
  },
  {
    name: 'cyan',
    kind: 'accent',
    group: 'blues',
    ramp: {
      light: [
        '#fafdfe',
        '#f2fafb',
        '#def7f9',
        '#caf1f6',
        '#b5e9f0',
        '#9ddde7',
        '#7dcedc',
        '#3db9cf',
        '#00a2c7',
        '#0797b9',
        '#107d98',
        '#0d3c48',
      ],
      dark: [
        '#0b161a',
        '#101b20',
        '#082c36',
        '#003848',
        '#004558',
        '#045468',
        '#12677e',
        '#11809c',
        '#00a2c7',
        '#23afd0',
        '#4ccce6',
        '#b6ecf7',
      ],
    },
  },
  {
    name: 'sky',
    kind: 'accent',
    group: 'blues',
    ramp: {
      light: [
        '#f9feff',
        '#f1fafd',
        '#e1f6fd',
        '#d1f0fa',
        '#bee7f5',
        '#a9daed',
        '#8dcae3',
        '#60b3d7',
        '#7ce2fe',
        '#74daf8',
        '#00749e',
        '#1d3e56',
      ],
      dark: [
        '#0d141f',
        '#111a27',
        '#112840',
        '#113555',
        '#154467',
        '#1b537b',
        '#1f6692',
        '#197cae',
        '#7ce2fe',
        '#a8eeff',
        '#75c7f0',
        '#c2f3ff',
      ],
    },
  },
  {
    name: 'teal',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#fafefd',
        '#f3fbf9',
        '#e0f8f3',
        '#ccf3ea',
        '#b8eae0',
        '#a1ded2',
        '#83cdc1',
        '#53b9ab',
        '#12a594',
        '#0d9b8a',
        '#008573',
        '#0d3d38',
      ],
      dark: [
        '#0d1514',
        '#111c1b',
        '#0d2d2a',
        '#023b37',
        '#084843',
        '#145750',
        '#1c6961',
        '#207e73',
        '#12a594',
        '#0eb39e',
        '#0bd8b6',
        '#adf0dd',
      ],
    },
  },
  {
    name: 'jade',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#fbfefd',
        '#f4fbf7',
        '#e6f7ed',
        '#d6f1e3',
        '#c3e9d7',
        '#acdec8',
        '#8bceb6',
        '#56ba9f',
        '#29a383',
        '#26997b',
        '#208368',
        '#1d3b31',
      ],
      dark: [
        '#0d1512',
        '#121c18',
        '#0f2e22',
        '#0b3b2c',
        '#114837',
        '#1b5745',
        '#246854',
        '#2a7e68',
        '#29a383',
        '#27b08b',
        '#1fd8a4',
        '#adf0d4',
      ],
    },
  },
  {
    name: 'green',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#fbfefc',
        '#f4fbf6',
        '#e6f6eb',
        '#d6f1df',
        '#c4e8d1',
        '#adddc0',
        '#8eceaa',
        '#5bb98b',
        '#30a46c',
        '#2b9a66',
        '#218358',
        '#193b2d',
      ],
      dark: [
        '#0e1512',
        '#121b17',
        '#132d21',
        '#113b29',
        '#174933',
        '#20573e',
        '#28684a',
        '#2f7c57',
        '#30a46c',
        '#33b074',
        '#3dd68c',
        '#b1f1cb',
      ],
    },
  },
  {
    name: 'grass',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#fbfefb',
        '#f5fbf5',
        '#e9f6e9',
        '#daf1db',
        '#c9e8ca',
        '#b2ddb5',
        '#94ce9a',
        '#65ba74',
        '#46a758',
        '#3e9b4f',
        '#2a7e3b',
        '#203c25',
      ],
      dark: [
        '#0e1511',
        '#141a15',
        '#1b2a1e',
        '#1d3a24',
        '#25482d',
        '#2d5736',
        '#366740',
        '#3e7949',
        '#46a758',
        '#53b365',
        '#71d083',
        '#c2f0c2',
      ],
    },
  },
  {
    name: 'mint',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#f9fefd',
        '#f2fbf9',
        '#ddf9f2',
        '#c8f4e9',
        '#b3ecde',
        '#9ce0d0',
        '#7ecfbd',
        '#4cbba5',
        '#86ead4',
        '#7de0cb',
        '#027864',
        '#16433c',
      ],
      dark: [
        '#0e1515',
        '#0f1b1b',
        '#092c2b',
        '#003a38',
        '#004744',
        '#105650',
        '#1e685f',
        '#277f70',
        '#86ead4',
        '#a8f5e5',
        '#58d5ba',
        '#c4f5e1',
      ],
    },
  },
  {
    name: 'lime',
    kind: 'accent',
    group: 'greens',
    ramp: {
      light: [
        '#fcfdfa',
        '#f8faf3',
        '#eef6d6',
        '#e2f0bd',
        '#d3e7a6',
        '#c2da91',
        '#abc978',
        '#8db654',
        '#bdee63',
        '#b0e64c',
        '#5c7c2f',
        '#37401c',
      ],
      dark: [
        '#11130c',
        '#151a10',
        '#1f2917',
        '#29371d',
        '#334423',
        '#3d522a',
        '#496231',
        '#577538',
        '#bdee63',
        '#d4ff70',
        '#bde56c',
        '#e3f7ba',
      ],
    },
  },
  {
    name: 'yellow',
    kind: 'accent',
    group: 'yellows',
    ramp: {
      light: [
        '#fdfdf9',
        '#fefce9',
        '#fffab8',
        '#fff394',
        '#ffe770',
        '#f3d768',
        '#e4c767',
        '#d5ae39',
        '#ffe629',
        '#ffdc00',
        '#9e6c00',
        '#473b1f',
      ],
      dark: [
        '#14120b',
        '#1b180f',
        '#2d2305',
        '#362b00',
        '#433500',
        '#524202',
        '#665417',
        '#836a21',
        '#ffe629',
        '#ffff57',
        '#f5e147',
        '#f6eeb4',
      ],
    },
  },
  {
    name: 'amber',
    kind: 'accent',
    group: 'yellows',
    ramp: {
      light: [
        '#fefdfb',
        '#fefbe9',
        '#fff7c2',
        '#ffee9c',
        '#fbe577',
        '#f3d673',
        '#e9c162',
        '#e2a336',
        '#ffc53d',
        '#ffba18',
        '#ab6400',
        '#4f3422',
      ],
      dark: [
        '#16120c',
        '#1d180f',
        '#302008',
        '#3f2700',
        '#4d3000',
        '#5c3d05',
        '#714f19',
        '#8f6424',
        '#ffc53d',
        '#ffd60a',
        '#ffca16',
        '#ffe7b3',
      ],
    },
  },
  {
    name: 'orange',
    kind: 'accent',
    group: 'yellows',
    ramp: {
      light: [
        '#fefcfb',
        '#fff7ed',
        '#ffefd6',
        '#ffdfb5',
        '#ffd19a',
        '#ffc182',
        '#f5ae73',
        '#ec9455',
        '#f76b15',
        '#ef5f00',
        '#cc4e00',
        '#582d1d',
      ],
      dark: [
        '#17120e',
        '#1e160f',
        '#331e0b',
        '#462100',
        '#562800',
        '#66350c',
        '#7e451d',
        '#a35829',
        '#f76b15',
        '#ff801f',
        '#ffa057',
        '#ffe0c2',
      ],
    },
  },
  {
    name: 'brown',
    kind: 'accent',
    group: 'metals',
    ramp: {
      light: [
        '#fefdfc',
        '#fcf9f6',
        '#f6eee7',
        '#f0e4d9',
        '#ebdaca',
        '#e4cdb7',
        '#dcbc9f',
        '#cea37e',
        '#ad7f58',
        '#a07553',
        '#815e46',
        '#3e332e',
      ],
      dark: [
        '#12110f',
        '#1c1816',
        '#28211d',
        '#322922',
        '#3e3128',
        '#4d3c2f',
        '#614a39',
        '#7c5f46',
        '#ad7f58',
        '#b88c67',
        '#dbb594',
        '#f2e1ca',
      ],
    },
  },
  {
    name: 'bronze',
    kind: 'accent',
    group: 'metals',
    ramp: {
      light: [
        '#fdfcfc',
        '#fdf7f5',
        '#f6edea',
        '#efe4df',
        '#e7d9d3',
        '#dfcdc5',
        '#d3bcb3',
        '#c2a499',
        '#a18072',
        '#957468',
        '#7d5e54',
        '#43302b',
      ],
      dark: [
        '#141110',
        '#1c1917',
        '#262220',
        '#302a27',
        '#3b3330',
        '#493e3a',
        '#5a4c47',
        '#6f5f58',
        '#a18072',
        '#ae8c7e',
        '#d4b3a5',
        '#ede0d9',
      ],
    },
  },
  {
    name: 'gold',
    kind: 'accent',
    group: 'metals',
    ramp: {
      light: [
        '#fdfdfc',
        '#faf9f2',
        '#f2f0e7',
        '#eae6db',
        '#e1dccf',
        '#d8d0bf',
        '#cbc0aa',
        '#b9a88d',
        '#978365',
        '#8c7a5e',
        '#71624b',
        '#3b352b',
      ],
      dark: [
        '#121211',
        '#1b1a17',
        '#24231f',
        '#2d2b26',
        '#38352e',
        '#444039',
        '#544f46',
        '#696256',
        '#978365',
        '#a39073',
        '#cbb99f',
        '#e8e2d9',
      ],
    },
  },
  {
    name: 'gray',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fcfcfc',
        '#f9f9f9',
        '#f0f0f0',
        '#e8e8e8',
        '#e0e0e0',
        '#d9d9d9',
        '#cecece',
        '#bbbbbb',
        '#8d8d8d',
        '#838383',
        '#646464',
        '#202020',
      ],
      dark: [
        '#111111',
        '#191919',
        '#222222',
        '#2a2a2a',
        '#313131',
        '#3a3a3a',
        '#484848',
        '#606060',
        '#6e6e6e',
        '#7b7b7b',
        '#b4b4b4',
        '#eeeeee',
      ],
    },
  },
  {
    name: 'mauve',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fdfcfd',
        '#faf9fb',
        '#f2eff3',
        '#eae7ec',
        '#e3dfe6',
        '#dbd8e0',
        '#d0cdd7',
        '#bcbac7',
        '#8e8c99',
        '#84828e',
        '#65636d',
        '#211f26',
      ],
      dark: [
        '#121113',
        '#1a191b',
        '#232225',
        '#2b292d',
        '#323035',
        '#3c393f',
        '#49474e',
        '#625f69',
        '#6f6d78',
        '#7c7a85',
        '#b5b2bc',
        '#eeeef0',
      ],
    },
  },
  {
    name: 'slate',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fcfcfd',
        '#f9f9fb',
        '#f0f0f3',
        '#e8e8ec',
        '#e0e1e6',
        '#d9d9e0',
        '#cdced6',
        '#b9bbc6',
        '#8b8d98',
        '#80838d',
        '#60646c',
        '#1c2024',
      ],
      dark: [
        '#111113',
        '#18191b',
        '#212225',
        '#272a2d',
        '#2e3135',
        '#363a3f',
        '#43484e',
        '#5a6169',
        '#696e77',
        '#777b84',
        '#b0b4ba',
        '#edeef0',
      ],
    },
  },
  {
    name: 'sage',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fbfdfc',
        '#f7f9f8',
        '#eef1f0',
        '#e6e9e8',
        '#dfe2e0',
        '#d7dad9',
        '#cbcfcd',
        '#b8bcba',
        '#868e8b',
        '#7c8481',
        '#5f6563',
        '#1a211e',
      ],
      dark: [
        '#101211',
        '#171918',
        '#202221',
        '#272a29',
        '#2e3130',
        '#373b39',
        '#444947',
        '#5b625f',
        '#63706b',
        '#717d79',
        '#adb5b2',
        '#eceeed',
      ],
    },
  },
  {
    name: 'olive',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fcfdfc',
        '#f8faf8',
        '#eff1ef',
        '#e7e9e7',
        '#dfe2df',
        '#d7dad7',
        '#cccfcc',
        '#b9bcb8',
        '#898e87',
        '#7f847d',
        '#60655f',
        '#1d211c',
      ],
      dark: [
        '#111210',
        '#181917',
        '#212220',
        '#282a27',
        '#2f312e',
        '#383a36',
        '#454843',
        '#5c625b',
        '#687066',
        '#767d74',
        '#afb5ad',
        '#eceeec',
      ],
    },
  },
  {
    name: 'sand',
    kind: 'gray',
    group: 'grays',
    ramp: {
      light: [
        '#fdfdfc',
        '#f9f9f8',
        '#f1f0ef',
        '#e9e8e6',
        '#e2e1de',
        '#dad9d6',
        '#cfceca',
        '#bcbbb5',
        '#8d8d86',
        '#82827c',
        '#63635e',
        '#21201c',
      ],
      dark: [
        '#111110',
        '#191918',
        '#222221',
        '#2a2a28',
        '#31312e',
        '#3b3a37',
        '#494844',
        '#62605b',
        '#6f6d66',
        '#7c7b74',
        '#b5b3ad',
        '#eeeeec',
      ],
    },
  },
]

export function scalesIn(group: GroupKey): RadixScale[] {
  return SCALES.filter((s) => s.group === group)
}

export type { Mode }
export { MODES }

// why: WCAG relative luminance → pick black/white label ink that stays legible
// across all 12 steps in both modes (steps 1–8 are pale in light, dark in dark).
export function inkOn(hex: string): string {
  const c = hex.replace('#', '')
  const toLin = (v: number) => (v <= 0.03928 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4)
  const r = toLin(Number.parseInt(c.slice(0, 2), 16) / 255)
  const g = toLin(Number.parseInt(c.slice(2, 4), 16) / 255)
  const b = toLin(Number.parseInt(c.slice(4, 6), 16) / 255)
  const lum = 0.2126 * r + 0.7152 * g + 0.0722 * b
  return lum > 0.42 ? '#11181c' : '#ffffff'
}

// why: a scale is "bright" when its step-9 solid needs dark text (sky, mint,
// lime, yellow, amber) — surfaced as a badge, not a group.
export function isBright(scale: RadixScale): boolean {
  return inkOn(scale.ramp.light[IDENTITY_STEP - 1]) === '#11181c'
}
