...
function hctEqual(a: HctTriplet, b: HctTriplet): boolean {
...
const HCT_UPDATE_EPSILON = 5e-3

function hctClose(a: HctTriplet, b: HctTriplet): boolean {
  return (
...
export function useHctFromHex(
  ...
  const update = (partial: Partial<HctTriplet>) => {
    setHct((cur) => {
      ...
      // why: short-circuit when the requested triplet is within solver
      // epsilon of current. See HCT_UPDATE_EPSILON comment.
      if (hctClose(cur, next)) return cur
      return next
    })
  }
