...
// dense rows with full contrast badges (incl. threshold). One walk per render
...
export function useOverrideData() {
  ...
  // unlike roles which evaluate against a single partner. Badge tracks the
  // worst (min) ratio and which partner produced it. `passes` ANDs across
  // partners — pass-on-one/fail-on-other still flags.
