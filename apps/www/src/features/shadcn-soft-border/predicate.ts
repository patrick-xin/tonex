...
// not a separate boolean in PortableTheme. The two roles being bound to
// outline-variant in BOTH modes IS the on state (single source of truth, no
// drift between toggle and binding). Asymmetric state (one mode soft, one
// hard) reads as off — the switch surfaces a global stylistic choice, so any
// asymmetry returns "indeterminate / off" rather than green-checking on a
// half-applied state. ADR-0022 rule 5: composition of core data into a
