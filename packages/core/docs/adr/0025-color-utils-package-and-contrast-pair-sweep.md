...
**Catalogue extensions live in code, not here.** Commitment 9's closed-list discipline means the pair set grows only by schema edits to `CONTRAST_PAIRS` (`pairs.ts`), and commitment 7 pre-baked the `intent` / `threshold` shape so those edits never migrate schema. Two such sweeps shipped (issue #47 non-text coverage gaps — `input`/`ring`/`destructive` on `--background`, outline across the surface-container ladder; and the link-variant *role-as-text* addendum — `--primary`/`--destructive` rendered AS text on neutral surfaces, the inverse of commitment 6's `-foreground / root` direction). Both are executable truth: the live count, the layer/intent splits, and the per-pair rationale are the `CONTRAST_PAIRS` tuple, its `// why:` / overview blocks, and the count assertions in `contrast.test.ts` — not prose mirrored here. The body's closed-list discipline (commitments 6–9) is what those sweeps obey; it is unchanged by them.

**Code anchors:** `packages/core/src/theme/contrast/contrast.test.ts`, `packages/color-utils/src/oklch.ts` — contrast-pair sweep + color-utils package.
