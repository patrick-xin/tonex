// why: Tailwind v4 default palette in OKLCH form. Single source of truth for
// the surface-tint base shades — `tint.ts` parses the OKLCH strings into argb
// once at module load. Hand-typed alongside Tailwind's own values; if Tailwind
// retunes a shade, mirror the change here. Includes the four extended neutrals
// (taupe, mauve, mist, olive) the project ships beyond Tailwind's defaults so
// the surface picker has a wider neutral menu.
//
// This file is data only — no transformation. Adding a palette: extend the
...
// why: neutral-ish palettes suitable for the surface picker. Subset of the
// full TW palette — the chromatic palettes (red, blue, etc.) wouldn't make
// sensible surface bases. The four tail entries (taupe, mauve, mist, olive)
// are project-extended neutrals beyond Tailwind's defaults.
export const NEUTRAL_PALETTE_NAMES = [
  'slate',
  'gray',
  'zinc',
  'neutral',
...
export type NeutralPaletteName = (typeof NEUTRAL_PALETTE_NAMES)[number]
